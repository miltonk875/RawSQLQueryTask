SELECT 
    ...
    SUM(CAST(IFNULL(ts.total_sold, 0) + IFNULL(market.market_orders, 0) AS UNSIGNED)) AS sales
...
LEFT JOIN (
    ...
    WHERE DATE(od.created_at) BETWEEN '2025-04-29' AND '2025-04-29'
...
LEFT JOIN (
    ...
    FROM bbbd_ecommerce_test.order_details AS od
    WHERE DATE(od.created_at) BETWEEN '2025-04-28' AND '2025-04-28'
    GROUP BY od.product_id
...
LEFT JOIN (
    ...
    WHERE 
        ...
        AND ts.type = 'sell'
        AND DATE(ts.created_at) BETWEEN '2025-04-29' AND '2025-04-29'
