SELECT 
    ...
    SUM(IFNULL(sales.total_sold, 0)) AS sales
...
LEFT JOIN (
    SELECT 
        ...
        END) AS current_sale,
        SUM(CASE 
            WHEN DATE(od.created_at) >= '2025-05-02' AND DATE(od.created_at) <='2025-05-02' THEN od.quantity
            ELSE 0 
        END) AS previous_sale,
        SUM(od.quantity) AS total_sold
    FROM bbbd_ecommerce_test.order_details AS od
    WHERE DATE(od.created_at) >= '2025-05-02' AND DATE(od.created_at) <='2025-05-02'
...
LEFT JOIN (
    ...
    FROM bbbd_ecommerce_test.order_details AS od
    WHERE DATE(od.created_at) >= '2025-05-03' AND DATE(od.created_at) <='2025-05-03'
    GROUP BY od.product_id
...
LEFT JOIN (
    ...
    WHERE 
        ...
        AND ts.type = 'sell'
        AND DATE(tsl.created_at) >= '2025-05-03' AND DATE(tsl.created_at) <='2025-05-03'
